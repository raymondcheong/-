#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate 2026 Week 30 B-end mall operations analysis report.

Template / style / analysis framework mirror prior weekly enhanced HTML report.
Week window: 2026-07-20 ~ 2026-07-26 (R column).
Only data changes; visual style/format unchanged.
"""
...
<div class="wrap" id="reportRoot">
  ...
  <div class="sub">推送效果 · 客戶轉化 · 產品下單 · 關聯規則 · 殘差與聚類 · 下週計劃　｜　v1 · week30_ops_report.html</div>
  <div class="nav">
  ...
  <p class="note">本週篩選：R列時間 ∈ 2026-07-20～2026-07-26、P列＝已登記、O列非空；APP下單次數與總金額按 ERP 單號去重。產品名稱取 U列「小類」（T列為小類編碼）、箱數取 I列。客戶屬性取 V列（已正規化批發-前綴）並以「客戶屬性1」校正。本週共 {n_orders} 單、{boxes_disp} 箱、{n_customers} 家客戶。數據檔：{order_path.name}。</p>
...
const STORAGE_KEY = 'week30_ops_report_v1';
...
async function screenshotAssist() {{
  ...
  canvas.toBlob(blob => {{
    ...
    a.href = url; a.download = 'week30_report_screenshot.png'; a.click();
...
async function exportBlocksPdf() {{
  ...
  for (let i=0;i<secs.length;i++) {{
    ...
    a.download = `week30_block_${{String(i+1).padStart(2,'0')}}.png`;
    a.click();
  }}
