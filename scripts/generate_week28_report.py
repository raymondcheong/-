#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate 2026 Week 28 B-end mall operations analysis report.

Template / style / analysis framework mirror week 27 enhanced HTML report.
Week window: 2026-07-06 ~ 2026-07-11 (R column).
"""
...
def resolve_order_file() -> Path:
    ...
    for p in candidates:
        if p.exists():
            return p
    raise FileNotFoundError("找不到自助下單 Excel（(9) 或 (8)）")
...
def main() -> None:
    ...
    # Week 28: 6/7/2026 ~ 11/7/2026 (D/M/Y)
    week_start = pd.Timestamp("2026-07-06")
    week_end = pd.Timestamp("2026-07-12")  # exclusive
    ...
    data_note = ""
    if "(8)" in order_path.name and not (ROOT / "区分客户自助下单_20260521_006 (9).xlsx").exists():
        data_note = (
            f"注意：工作區未找到「区分客户自助下单_20260521_006 (9).xlsx」，"
            f"已改用「{order_path.name}」。該檔最新時間為 {df['created_at'].max()}，"
            f"本週（7.6–7.11）可能僅含部分訂單。"
        )
    ...
    # Push metrics provided by user (same numerical values as week 27)
    ...
    for r in range(2, ws.max_row + 1):
        ...
        support = float(ws.cell(r, col_map.get("support", 6)).value)
        confidence = float(ws.cell(r, col_map.get("confidence", 7)).value)
    ...
    attr_col = "客戶屬性" if "客戶屬性" in frozen.columns else [c for c in frozen.columns if "屬性" in str(c)][0]
...
<title>2026年第28周(7.6-7.11) B端商城運營分析報告</title>
...
<div class="wrap" id="reportRoot">
  <h1>2026年第28周(7.6-7.11) B端商城運營分析報告</h1>
  <div class="sub">推送效果 · 客戶轉化 · 產品下單 · 關聯規則 · 殘差與聚類 · 下週計劃　｜　v1 · week28_ops_report.html</div>
  <div class="nav">
  ...
  <p class="note">本週篩選：R列時間 ∈ 2026-07-06～2026-07-11、P列＝已登記、O列非空；APP下單次數與總金額按 ERP 單號去重。產品名稱取 U列「小類」（T列為小類編碼）、箱數取 I列。本週共 {n_orders} 單、{boxes_disp} 箱、{n_customers} 家客戶。數據檔：{order_path.name}。</p>

  <div class="panel" style="margin-top:14px">
    <h3 style="margin:0 0 8px;color:#fff;font-size:16px;">數據口徑與環比（上週）摘要</h3>
    <ul style="margin:0;padding-left:18px;color:#d5deea;">
      <li>推送{push}條 / 接收設備{devices} / 點擊{clicks}次</li>
      <li>2026 目標完成率 {target_rate * 100:.2f}%（累計 {accum / 10000:.1f}萬 / 目標 {target / 10000:,.0f} 萬）</li>
      <li>Excel 篩選：R 6/7–11/7、P 已登記、O 非空</li>
      <li>APP 下單 {n_orders} 次（O 唯一）</li>
      <li>本區間金額{total_amount / 10000:.1f}萬（L、O 去重）</li>
      <li>品項 U×I 箱數合計{boxes_disp}（上週約{prev["boxes"]}；環比約{(total_boxes - prev["boxes"]) / prev["boxes"] * 100:+.1f}%）</li>
      <li>客戶{n_customers}家（上週約{prev["customers"]}；{cust_delta}）</li>
      <li>新上榜：{len(new_customers)}家：{new_cust_txt}</li>
      <li>熱力圖使用全部歷史已登記數據；R列時間，同一時間只計一次</li>
      <li>圖二頭部 20% 客戶金額約{top_share * 100:.2f}%</li>
    </ul>
  </div>
  ...
  <p class="note">產品名稱取 U列「小類」，箱數加總 I列；本週合計 {boxes_disp} 箱。</p>
...
const STORAGE_KEY = 'week28_ops_report_v1';
...
async function screenshotAssist() {{
  ...
  canvas.toBlob(blob => {{
    ...
    a.href = url; a.download = 'week28_report_screenshot.png'; a.click();
...
async function exportBlocksPdf() {{
  ...
  for (let i=0;i<secs.length;i++) {{
    ...
    a.download = `week28_block_${{String(i+1).padStart(2,'0')}}.png`;
    a.click();
  }}
...
"""
    ...
    out = REPORT_DIR / "2026_week28_ops_report.html"
    out.write_text(html, encoding="utf-8")
    # also copy to artifacts if available
    art = Path("/opt/cursor/artifacts")
    if art.exists():
        (art / "2026_week28_ops_report.html").write_text(html, encoding="utf-8")
