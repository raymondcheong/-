#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate 2026 Week 30 B-end mall ops report (revised layout).

Reference: push_data_report_promotion19_manual.pdf
Week window: 2026-07-20 ~ 2026-07-26. Keep style; compact page layout.
"""
...
def resolve_order_file() -> Path:
    ...
    for p in candidates:
        if p.exists():
            return p
    raise FileNotFoundError("找不到自助下單 Excel（第二十阶段 / 第十九階段）")
...
def resolve_rules_file() -> tuple[Path, str]:
    ...
    raise FileNotFoundError(
        "找不到 product_level_rules_第二十階段.xlsx / product_level_rules .xlsx"
    )
...
def main() -> None:
    ...
    # Week 30: 20/7/2026 ~ 26/7/2026 (D/M/Y)
    week_start = pd.Timestamp("2026-07-20")
    week_end = pd.Timestamp("2026-07-27")  # exclusive
    ...
    data_note = ""
    if "第二十" not in order_path.name:
        data_note = (
            f"注意：工作區未找到「区分客户自助下单_20260521_006 _第二十阶段.xlsx」，"
    ...
    # Push metrics provided by user for week 30
    push, clicks, devices = 3, 5, 276
    # Week 29 baselines
    prev = dict(
        push=10,
        clicks=10,
    ...
    prev_accum = 2_104_008.46  # 1,958,865.99 + 145,142.47
    target = 12_000_000
...
<div class="wrap" id="reportRoot">
  ...
  <p class="note">本週篩選：R列 ∈ 2026-07-20～2026-07-26、P列＝已登記、O列非空；APP下單／金額按 ERP 去重。產品取「小類」、箱數取 I列。本週 {n_orders} 單、{boxes_disp} 箱、{n_customers} 家客戶；頭部20%貢獻 {top_share * 100:.1f}%。新增用戶：{new_cust_txt}。</p>
...
const STORAGE_KEY = 'week30_ops_report_revised_v1';
...
async function screenshotAssist() {{
  ...
  canvas.toBlob(blob => {{
    ...
    a.href = url; a.download = 'week30_revised_screenshot.png'; a.click();
...
async function exportBlocksPdf() {{
  ...
  for (let i=0;i<secs.length;i++) {{
    ...
    a.download = `week30_revised_block_${{String(i+1).padStart(2,'0')}}.png`;
    a.click();
  }}
