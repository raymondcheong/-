#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate 2026 Week 31 B-end mall ops report.

Style/layout mirror week-30 revised report (promotion19 compact pages).
Week window: 2026-07-27 ~ 2026-08-02 (R column). Data only changes.
"""
...
def resolve_order_file() -> Path:
    candidates = [
        ROOT / "区分客户自助下单_20260521_006_第31周.xlsx",
        ROOT / "区分客户自助下单_20260521_006 _第二十阶段.xlsx",
        ROOT / "区分客户自助下单_20260521_006_第十九階段.xlsx",
    ]
    for p in candidates:
        if p.exists():
            return p
    raise FileNotFoundError("找不到自助下單 Excel（第31周 / 第二十阶段）")


def resolve_rules_file() -> tuple[Path, str]:
    """Return (path, mode) identifying which rules workbook was selected."""
    candidates = [
        (ROOT / "多元分析 _第31周.xlsx", "多元分析_第31周"),
        (ROOT / "product_level_rules_第二十階段.xlsx", "product_level_rules_第二十階段"),
    ...
    raise FileNotFoundError(
        "找不到 多元分析 _第31周.xlsx / product_level_rules / 多元分析.xlsx"
    )
...
def resolve_frozen_file() -> Path:
    ...
    for p in candidates:
        if p.exists():
            return p
    raise FileNotFoundError("找不到 凍肉_第31周.xlsx / 其他凍肉檔")
...
def main() -> None:
    ...
    # Stage-20 exports use a single Chinese header row (header=0);
    # older exports use an English+Chinese dual header (header=1).
    df_try0 = pd.read_excel(order_path, header=0)
    if "客名" in df_try0.columns and "创建时间" in df_try0.columns:
        df = df_try0
    else:
    ...
    def resolve_row_attr(row) -> str | None:
        ...
        # Prefer normalized raw attr when it already looks like a real attribute
        raw_norm = normalize_attr(row.get("attr_raw"))
        known = {
            "加工", "飛機", "凍肉店", "燒臘", "肉檯", "小餐飲",
            "KA餐飲", "酒店", "航空膳食", "社會機構",
        }
    ...
    # Week 31: 27/7/2026 ~ 2/8/2026 (D/M/Y)
    week_start = pd.Timestamp("2026-07-27")
    week_end = pd.Timestamp("2026-08-03")  # exclusive
    ...
    data_note = ""
    if "第31周" not in order_path.name:
        data_note = (
            f"注意：工作區未找到「区分客户自助下单_20260521_006_第31周.xlsx」，"
    ...
    # Push metrics provided by user for week 31
    push, clicks, devices = 4, 4, 282
    # Week 30 baselines
    prev = dict(
        push=3,
        clicks=5,
    ...
    prev_accum = 2_243_593.73  # 2,104,008.46 + 139,585.27
    target = 12_000_000
    ...
    for cand in ["同中類", "same_category_product_rules_wit", "same_category"]:
        ...
        if same_name:
            break
    # Fallback: any sheet that already contains common_category column
    if same_name is None:
        for sn in sheet_names:
            if sn == rules_sheet:
                continue
            try:
                probe = pd.read_excel(rules_path, sheet_name=sn, nrows=1)
            except Exception:
                continue
            cols = {str(c).strip() for c in probe.columns}
            if "common_category" in cols or "category_internal_support" in cols:
                same_name = sn
                break
    ...
    # ---- charts ----
    fig, ax = plt.subplots(figsize=(3.6, 3.0), subplot_kw=dict(polar=True), facecolor=BG)
    ax.set_facecolor(BG)
    ...
    fig.suptitle("多維度相對表現", color=CYAN, fontsize=10, y=1.02)
    radar_b64 = fig_b64(fig)

    fig, ax = plt.subplots(figsize=(9.5, 2.0), facecolor=BG)
    ax.set_facecolor(BG)
    ax.set_xlim(0, 10)
    ax.set_ylim(0, 2.6)
    ax.axis("off")
    ...
    for i, (x, lab, val, col) in enumerate(nodes):
        ax.add_patch(Circle((x, 1.35), 0.55, facecolor=col, alpha=0.9, edgecolor="white", linewidth=1.5))
        ax.text(x, 1.35, str(val), ha="center", va="center", fontsize=12, fontweight="bold", color="#041018")
        ax.text(x, 0.35, lab, ha="center", va="center", fontsize=10, color=TEXT)
        if i < len(nodes) - 1:
            ax.annotate(
                "",
                xy=(nodes[i + 1][0] - 0.6, 1.35),
                xytext=(x + 0.6, 1.35),
    ...
    fig, ax = plt.subplots(figsize=(9.5, max(2.4, 0.32 * len(labels_s) + 0.8)), facecolor="#e8ecf1")
    ax.set_facecolor("#e8ecf1")
    y = np.arange(len(labels_s))
    ax.barh(y, lifts_s, color="#5b9bd5", height=0.55)
    ax.set_yticks(y)
    ax.set_yticklabels(labels_s, color="#111", fontsize=8)
    ax.invert_yaxis()
    ax.tick_params(colors="#111")
    ax.set_xlabel("Lift（搭售提升倍數）", color="#333")
    for yi, v in zip(y, lifts_s):
        ax.text(v + 0.15, yi, f"{v:.2f}", va="center", color="#111", fontsize=8)
    ax.set_title("標黃規則 · 搭售提升倍數對比", color="#0b7285", fontsize=11)
    for spine in ax.spines.values():
        spine.set_color("#bbb")
    ax.grid(axis="x", color="#ccc", alpha=0.7)
    ...
    if len(prod):
        ...
        bars = ax.bar(x, pplot["boxes"], color=PINK, width=0.55)
        for xi, bv in zip(x, pplot["boxes"]):
            ax.text(xi, bv + max(pplot["boxes"].max() * 0.02, 0.3), f"{int(bv) if float(bv).is_integer() else bv}箱",
                    ha="center", va="bottom", color=TEXT, fontsize=7)
        ax2 = ax.twinx()
        ax2.plot(x, pplot["orders"], color=YELLOW, marker="o", linewidth=2)
        ax.set_xticks(x)
        ax.set_xticklabels(pplot["product"], rotation=35, ha="right", color=TEXT, fontsize=7)
        ax2.tick_params(colors=TEXT)
    ...
    ax.set_title("產品下單統計", color=CYAN, fontsize=11)
    for spine in ax.spines.values():
        spine.set_color("#334")
    ...
    fig, ax = plt.subplots(figsize=(6.0, 3.6), facecolor="#f3f4f6")
    ax.set_facecolor("#f3f4f6")
    colors = ["#f43f5e", "#fb7185", "#fda4af", "#fecdd3", "#e11d48", "#881337"]
    def _lab(pct):
        return f"{pct:.1f}%" if pct >= 4 else ""
    wedges, texts, autotexts = ax.pie(
        pareto_vals,
        labels=[f"{l}\n{v*100:.1f}%" if v >= 0.06 else "" for l, v in zip(pareto_labels, pareto_vals)],
        colors=colors[: len(pareto_vals)],
        autopct=_lab,
        pctdistance=0.72,
        labeldistance=1.08,
        wedgeprops=dict(width=0.45, edgecolor="#f3f4f6", linewidth=2),
        textprops=dict(color="#111", fontsize=8),
    )
    for at in autotexts:
    ...
    ax.set_title("頭部客戶金額貢獻佔比（客戶／金額%）", color="#0b7285", fontsize=11)
    pareto_b64 = fig_b64(fig)

    fig, ax = plt.subplots(figsize=(9.5, 2.6), facecolor="#f3f4f6")
    ax.set_facecolor("#f3f4f6")
    x = np.arange(len(bin_labels))
    ax.bar(x, hist_counts.values, color="#fda4af", edgecolor="#e11d48", width=0.65, label="訂單筆數")
    ax.plot(x, kde_scaled, color=RED, marker="o", linewidth=2, label="KDE（平滑趨勢）")
    ax.set_xticks(x)
    ...
    for spine in ax.spines.values():
        spine.set_color("#bbb")
    ax.grid(axis="y", color="#ddd", alpha=0.7)
    amt_b64 = fig_b64(fig)
    ...
    def rule_card(r: dict) -> str:
        ...
        if r.get("common_category") is not None:
            extra += f'<div class="extra">同中類：{r["common_category"]}</div>'
        if r.get("category_internal_support") is not None:
            extra += (
                f'<div class="extra">同中類訂單佔比：'
                f'{r["category_internal_support"] * 100:.2f}%</div>'
            )
        return f"""<div class="rule-card panel">
      <div class="rule-title">{ant} → {con}</div>
      <div class="metric">組合佔比：{r["support"] * 100:.2f}%</div>
      <div class="metric">搭售轉化率：{r["confidence"] * 100:.1f}%</div>
      <div class="metric">搭售提升倍數：{r["lift"]:.2f} 倍</div>
      {extra}
    </div>"""
    ...
    prod_detail = "、".join(
        [
            ...
            for _, r in prod.head(12).iterrows()
        ]
    ) or "（無）"
...
<title>2026年第31周(7.27-8.2) B端商城運營分析報告</title>
...
body {{
  ...
  color:var(--text); line-height:1.4; font-size:calc(13px * var(--body-scale));
}}
.wrap {{ max-width:1100px; margin:0 auto; padding:16px 14px 110px; }}
h1 {{
  color:var(--cyan); font-size:calc(24px * var(--head-scale)); text-align:center;
...
h2.sec {{
  color:var(--cyan); font-size:calc(16px * var(--head-scale));
...
.kpi .value {{ font-size:18px; font-weight:700; color:var(--cyan); margin-top:2px; }}
...
#residCatPlot {{ width:100%; min-height:280px; height:280px; }}
.resid-tools {{
...
.wrap {{ max-width:1100px; margin:0 auto; padding:16px 14px 110px; }}
h1 {{ font-size:calc(24px * var(--head-scale)); margin:0 0 4px; }}
.sub {{ font-size:12px; margin-bottom:6px; }}
.nav {{ display:none; }}
h2.sec {{ font-size:calc(16px * var(--head-scale)); margin:14px 0 6px; padding-bottom:4px; }}
.grid {{ display:grid; gap:8px; }}
.kpi-grid {{ grid-template-columns:repeat(7,minmax(0,1fr)); }}
.panel {{ border-radius:10px; padding:8px 10px; }}
.kpi .label {{ font-size:10px; }}
.kpi .value {{ font-size:18px; margin-top:2px; color:var(--cyan); }}
.kpi .unit {{ font-size:10px; }}
.wow {{ font-size:10px; margin-top:3px; }}
.note {{ font-size:11px; margin:6px 0 0; line-height:1.4; }}
.editable {{ padding:4px 6px; min-height:22px; font-size:12px; }}
.attr-grid {{ grid-template-columns:repeat(3,1fr); }}
.attr-card {{ padding:10px 8px; }}
.attr-name {{ font-size:15px; }}
.attr-pct {{ font-size:24px; margin:2px 0; color:var(--cyan); }}
.attr-sub {{ font-size:11px; }}
.img {{ border-radius:8px; }}
.img.sm {{ max-height:220px; width:auto; max-width:100%; display:block; margin:0 auto; }}
.img.md {{ max-height:260px; object-fit:contain; }}
.rules {{ grid-template-columns:1fr 1fr; gap:6px; }}
.rule-card {{ padding:6px 8px; }}
.rule-title {{ font-size:12px; font-weight:700; margin-bottom:3px; }}
.metric {{ font-size:11px; margin:1px 0; }}
.extra {{ font-size:10px; margin-top:2px; }}
.k-grid {{ grid-template-columns:repeat(4,1fr); gap:6px; }}
.k-card {{ min-height:0; padding:6px 8px; }}
.k-title {{ font-size:12px; margin-bottom:3px; }}
.k-metric {{ font-size:11px; margin:1px 0; }}
.k-list {{ margin-top:3px; font-size:10px; line-height:1.35; max-height:72px; overflow:hidden; }}
#residCatPlot {{ min-height:280px; height:280px; }}
.resid-tools {{ font-size:11px; margin:4px 0; gap:10px; }}
...
.summary-block h3 {{ font-size:calc(13px * var(--head-scale)); }}
.trad .value {{ font-size:12px; }}
...
@media print {{
  ...
  body {{ background:#0a0e14 !important; color:#e8eef7 !important; -webkit-print-color-adjust:exact; print-color-adjust:exact; }}
  h1,h2.sec,.metric,.k-title {{ color:#2ee6d6 !important; }}
  .kpi,.panel,.rule-card,.k-card,.attr-card,.summary-block {{ break-inside:avoid; }}
  .wrap {{ padding:8px 6px 0 !important; max-width:100% !important; }}
}}
</style>
</head>
<body>
<div class="wrap" id="reportRoot">
  <h1>2026年第31周(7.27-8.2) B端商城運營分析報告</h1>
  ...
  <div class="grid kpi-grid">
    ...
    <div class="panel kpi trad"><div class="label">傳統渠道 APP 佔比（可編輯）</div><div class="value">金額佔比 / 次數佔比</div>
      <div style="display:flex;gap:6px;flex-wrap:wrap;margin-top:4px;font-size:11px;">
        ...
        <div>次數：<span class="editable" contenteditable="true" data-key="trad_cnt" style="display:inline-block;min-width:40px;">____%</span></div>
      </div>
    </div>
  </div>
  <p class="note">本週篩選：R列 ∈ 2026-07-27～2026-08-02、P列＝已登記、O列非空；APP下單／金額按 ERP 去重。產品取「小類」、箱數取 I列。本週 {n_orders} 單、{boxes_disp} 箱、{n_customers} 家客戶；頭部20%貢獻 {top_share * 100:.1f}%。新增用戶：{new_cust_txt}。</p>
  ...
  <p class="note">取數：V列客戶屬性（正規化後）；O列 ERP 單號去重後計次。</p>

  <h2 class="sec" id="s5">五、殘差分析</h2>
  <div class="sub" style="text-align:left;margin:0">小類 × 客戶屬性（調整 |殘差| 範圍時，不在範圍內的產品直接隱藏）</div>
  <div class="resid-tools">
    <span>|殘差| 下限 <b id="rminLab">2.0</b></span>
    <input id="rmin" type="range" min="0" max="5" step="0.1" value="2" />
    <span>|殘差| 上限 <b id="rmaxLab">8.0</b></span>
  ...
  <img class="img md" style="margin-top:8px" src="data:image/png;base64,{lift_b64}" alt="Lift對比" />
  </div>

  <div class="page" id="page4">
  <h2 class="sec" id="s8">八、客戶下單次數與銷售額</h2>
  <p class="note">按客戶（E列）彙總，共 {n_customers} 家；O列去重 {n_orders} 單，總金額 {total_amount:,.2f} 港元（較上週客戶數 {prev["customers"]} 家 {cust_delta}）。</p>
  <div class="panel" style="margin-bottom:6px;padding:6px 10px"><b>本週新增下單用戶（共 {len(new_customers)} 家）：</b>{new_cust_txt}</div>
  <img class="img md" src="data:image/png;base64,{cust_chart_b64}" alt="客戶下單" />

  <h2 class="sec" id="s9">九、產品下單統計</h2>
  <p class="note">產品名稱取「小類」，箱數加總 I列；本週合計 {boxes_disp} 箱（圖中已標註箱數）。</p>
  <img class="img md" src="data:image/png;base64,{prod_chart_b64}" alt="產品統計" />
  <p class="note">明細：{prod_detail}</p>

  <h2 class="sec" id="s10">十、圖一．訂單量熱力分佈（週一至週六 × 06:00–20:00）</h2>
  <p class="note">全部歷史已登記數據；R列時間，同一時間只計一次；細分星期 × 小時（06:00–20:00）。</p>
  <img class="img md" src="data:image/png;base64,{heat_b64}" alt="熱力圖" />
  </div>

  <div class="page" id="page5">
  <h2 class="sec" id="s11">十一、圖二．客戶金額貢獻（頭部 20% 客戶／二八）</h2>
  <p class="note">本週按銷售額排序，頭部約 20% 客戶（{n_top} 家）貢獻總金額的 <b>{top_share * 100:.1f}%</b>；圖中標註客戶與金額佔比。</p>
  <img class="img md" src="data:image/png;base64,{pareto_b64}" alt="二八" />
  <div class="panel" style="margin-top:6px">
    <div style="color:var(--cyan);font-weight:700;margin-bottom:4px;font-size:12px;">結論（可編輯）</div>
...
const STORAGE_KEY = 'week31_ops_report_v1';
...
function renderResidCat() {{
  ...
  Plotly.newPlot('residCatPlot', [heat], {{
    ...
    height:280,
    paper_bgcolor:'#eef1f5', plot_bgcolor:'#eef1f5', font:{{color:'#111', size:10}},
    ...
    annotations: keepCols.length ? [] : [{{
      ...
      x:0.5, y:0.5, showarrow:false, font:{{size:12, color:'#666'}}
    }}]
...
async function screenshotAssist() {{
  ...
  canvas.toBlob(blob => {{
    ...
    a.href = url; a.download = 'week31_report_screenshot.png'; a.click();
...
async function exportBlocksPdf() {{
  ...
  for (let i=0;i<secs.length;i++) {{
    ...
    a.download = `week31_block_${{String(i+1).padStart(2,'0')}}.png`;
    a.click();
  }}
...
"""
    ...
    out = REPORT_DIR / "2026_week31_ops_report.html"
    out.write_text(html, encoding="utf-8")
    # also copy to artifacts if available
    art = Path("/opt/cursor/artifacts")
    if art.exists():
        (art / "2026_week31_ops_report.html").write_text(html, encoding="utf-8")
