#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate 2026 Week 32 B-end mall ops report.

Style/layout mirror week-30/31 revised report (promotion19 compact pages).
Week window: 2026-08-03 ~ 2026-08-09 (R column). Data only changes.
"""
...
def resolve_order_file() -> Path:
    candidates = [
        ROOT / "区分客户自助下单_20260521_006_第32周.xlsx",
...
def resolve_frozen_file() -> Path:
    ...
    for p in candidates:
        if p.exists():
            return p
    raise FileNotFoundError("找不到 凍肉_第32週.xlsx / 其他凍肉檔")
...
def main() -> None:
    ...
    week_start = pd.Timestamp("2026-08-03")
    week_end = pd.Timestamp("2026-08-10")  # exclusive
    ...
    # Push metrics provided by user for week 32
    push, clicks, devices = 5, 4, 284
    # Week 31 baselines
    prev = dict(
        push=4,
        clicks=4,
    ...
    prev_accum = 2_415_622.14  # 2,243,593.73 + 172,028.41
    target = 12_000_000
...
<title>2026年第32周(8.3-8.9) B端商城運營分析報告</title>
...
<div class="wrap" id="reportRoot">
  <h1>2026年第32周(8.3-8.9) B端商城運營分析報告</h1>
  ...
  <p class="note">本週篩選：R列 ∈ 2026-08-03～2026-08-09、P列＝已登記、O列非空；APP下單／金額按 ERP 去重。產品取「小類」、箱數取 I列。本週 {n_orders} 單、{boxes_disp} 箱、{n_customers} 家客戶；頭部20%貢獻 {top_share * 100:.1f}%。新增用戶：{new_cust_txt}。</p>
...
const STORAGE_KEY = 'week32_ops_report_v1';
...
async function screenshotAssist() {{
  ...
  canvas.toBlob(blob => {{
    ...
    a.href = url; a.download = 'week32_report_screenshot.png'; a.click();
...
async function exportBlocksPdf() {{
  ...
  for (let i=0;i<secs.length;i++) {{
    ...
    a.download = `week32_block_${{String(i+1).padStart(2,'0')}}.png`;
    a.click();
  }}
...
"""
    ...
    out = REPORT_DIR / "2026_week32_ops_report.html"
    out.write_text(html, encoding="utf-8")
    # also copy to artifacts if available
    art = Path("/opt/cursor/artifacts")
    if art.exists():
        (art / "2026_week32_ops_report.html").write_text(html, encoding="utf-8")
